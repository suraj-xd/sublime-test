  ...
  /*--------------------------------------------------------------------------------------------------------------------------*/
int ans = 0;
bool takenCol[9],takenDiagonal1[18],takenDiagonal2[19];
void helper(string board[8],int r,int &ans){
  if(r==8){
    ans++;
    return;
  }
  for(int c = 0;c < 8;c++){
    if(board[r][c]=='.'){
      if(!takenCol[c] and !takenDiagonal1[r-c+8-1] and !takenDiagonal2[r+c]){
        takenCol[c] = takenDiagonal1[r-c+8-1]  =takenDiagonal2[r+c] = true;
        helper(board,r+1,ans);
        takenCol[c] = takenDiagonal1[r-c+8-1]  =takenDiagonal2[r+c] = false;
      }
    }
  }
}
...
// Different Type of queen



int N,cnt = 0;
bool isSafe(int row,int col,int prevr,int prevc){
  if(prevc==col) return false;
  if(abs(prevr-row)==abs(prevc-col)) return false;
  if((abs(prevr-row)==2 and abs(prevc-col)==1) or (abs(prevr-row)==1 and abs(prevc-col)==2)) return false;
  return true;
}
