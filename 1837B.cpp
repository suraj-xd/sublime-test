...
// use: vector<pair<char,int>> encoded = run_length_encoding<char>(s);
...
void solve(){
  ...
  string s; cin >> s;
  int n = sz(s);
  vector<pair<char,int>> encoded = run_length_encoding<char>(s);
  for(int i = 0;i < sz(encoded);i++){
    if(i==0 and encoded[i].first=='?') encoded[i].first = '0';
    if(i==n-1 and encoded[i].first=='?') encoded[i].first = '?';
    if(i > 0 and i < n-1 and encoded[i].first=='?'){
        int a = encoded[i-1].first;
        int b = encoded[i].first;
        int c = encoded[i+1].first;
  ...
  for(int i = 0;i < sz(encoded);i++){
    rep(j,0,encoded[i].second) cout << encoded[j].first;
  }
  cout << nline;
